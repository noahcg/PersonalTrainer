...
export function StatCard({
  ...
  return (
    <Card
      className={cn(
        "group p-5 transition hover:-translate-y-1",
        tone === "dark" && "border-charcoal-900 bg-charcoal-950 text-ivory-50",
        tone === "sage" && "bg-sage-100/85",
      )}
    >
      <div className="flex items-start justify-between">
        <p className={cn("text-sm text-stone-500", tone === "dark" && "text-ivory-50/55")}>{label}</p>
      ...
      <p className="mt-5 font-serif text-4xl font-semibold">{value}</p>
