...
create policy "messages clients mark trainer messages read" on public.messages for update using (client_id = public.current_client_id() and kind = 'message' and sender_profile_id is distinct from auth.uid()) with check (client_id = public.current_client_id() and kind = 'message' and sender_profile_id is distinct from auth.uid());
create policy "messages trainers mark client messages read" on public.messages for update using (trainer_id = public.current_trainer_id() and kind = 'message' and sender_profile_id is distinct from auth.uid()) with check (trainer_id = public.current_trainer_id() and kind = 'message' and sender_profile_id is distinct from auth.uid());
